...
void Draw8point_Ellipse(HDC hdc,int xc,int yc,int x,int y,COLORREF color)
{
    SetPixel(hdc,xc+x,yc+y,color);
    SetPixel(hdc,xc+x,yc-y,color);
    SetPixel(hdc,xc-x,yc+y,color);
...
void Draw_Ellipse_Direct(HDC hdc, int xc, int yc, int A,int B, COLORREF color )
{
    ...
    {
        int x = 0, y = B;
        Draw8point_Ellipse(hdc,xc, yc, x, y, color);
        while (x < y)
        {
            x++;
...
static int xm1,ym1,xm2,ym2,r,menu=0,r1=100,r2=50,A=500,B=400, R,L=0;
static Shape node[100];
HDC hdc;
ofstream shape("shapefile.txt");
